...
import ecs.tests.DataMap;
import eu.larkc.csparql.cep.esper.WinList;
import eu.larkc.csparql.common.RDFTriple;
import eu.larkc.csparql.core.cichlid.Index;
import eu.larkc.csparql.core.cichlid.RDFS;
...
public class CsparqlEngineImpl implements Observer, CsparqlEngine {
	...
	private int win;
	private int step;
	private LinkedList<ReasonResult> queue = new LinkedList<>();
	...
	public CsparqlQueryResultProxy registerQuery(String command, boolean activateInference) throws ParseException {
		...
		if (activateInference) {
			logger.debug("eu.larkc.csparql.core.cichlid.RDFS reasoner");
	...
	public void registerWinAndStep(int win, int step) {
		this.win = win;
		this.step = step;
	}

	public void registerQueryId(int queryId) {
...
//		}
		...
		reasonResult.setTime(win);
		System.out.println("输入三元组个数: " + quads.size());
		Set<RDFTriple> reasonSet = RDFS.reason(quads);
		Set<RDFTriple> clSet = Index.getRelaRDF(reasonSet, this.indexList);
		Set<RDFTriple> blSet = Index.getListSet(clSet, this.indexList);
		reasonResult.setTripleSet(blSet);
		queue.add(reasonResult);
		Set<RDFTriple> in = new HashSet<>();
		for (ReasonResult res: queue) {
			in.addAll(res.tripleSet);
		}
		System.out.println("输入查询引擎个数: " + in.size());
		long ecsTime = BigQueryTests.query(queryId, in);
		...
		if (js) {
			if (wait > 0) {
				...
				all = all +  (end - start);
				ecs += ecsTime;
				System.out.println("平均ECS时间:" + (ecs / index));
				System.out.println("平均执行时间:" + (all / index));
				index++;
				System.out.println();
			}
		} else {
